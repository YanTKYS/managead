...
閉域ネットワーク向けの **Active Directory 参照専用確認支援ツール（v1.0.0 本番初版）** です。

> **重要**: ManageAdTool v1.0.0 は本番初版です。実運用担当者による v0.9.7 受入テストで重大な問題が確認されなかったため、参照専用 AD 確認支援ツールとしてリリースします。  
> 本ツールは `DirectoryReadOnly` による実 AD 参照のみを行います。AD 更新・削除・無効化・退職処理・OU 移動・グループ変更・GPO 編集は行いません。
