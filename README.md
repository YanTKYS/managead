...
閉域ネットワーク向けの **Active Directory 参照専用確認支援ツール（v1.0.1）** です。

> **重要**: 本ツールは `DirectoryReadOnly` による実 AD 参照のみを行います。AD 更新・削除・無効化・退職処理・OU 移動・グループ変更・GPO 編集は行いません。
