...
**本ツールは参照専用**であり、実AD更新は行いません。

## 方針（重要）
- 実AD更新機能は実装しません。
- 管理者権限ユーザーやサービスアカウントを使った AD 更新は対象外です。
- グループ追加・削除、GPO 編集、ユーザー無効化、退職処理、OU 移動、一括更新は実装しません。
...
- v0.3.0 の編集者ログイン・Domain Admins 判定は実AD環境での動作検証が必要です。
- 実 AD 更新は実行しません。書き込みモードは実装しない方針です。
...
- 実 AD 更新機能は今後も実装しない方針です。
- グループ追加・削除、GPO 編集、ユーザー無効化、退職処理、OU 移動、一括更新は**対象外**です。
- 既存の mail / department / title 画面は属性表示・変更予定確認用途に限定し、更新実行機能は無効化しています。
