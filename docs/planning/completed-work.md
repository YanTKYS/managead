...
- **UserEditPolicyService テスト**: ユーザー未選択、除外ユーザー、許可 OU 外、`EditableAttributes` 不足、条件充足時のみ編集可を検証。
- **LogReader テスト**: 存在しないファイル、壊れた JSON 行、`maxRows` 上限、`maxRows <= 0`、password キーマスクを検証。
- **ChangeSet / InMemoryAdService テスト**: ユーザー・コンピュータ差分生成、LDAP 属性名、検索、グループメンバー取得、GPO ダミー結果を検証。
