# ManageAdTool ロードマップ

## 方針

本ツールは閉域ネットワーク向けの **Active Directory 参照・限定編集支援ツール** として開発しています。

- 参照機能を主目的とし、安全性・シンプルさを優先します
- ユーザー書き込みは mail / displayName / sn / givenName のみに限定します
- コンピュータ書き込みは description のみに限定します（v0.5.0 以降）
- グループ書き込みはユーザーのメンバー追加・削除のみに限定します（v0.6.0 以降）
- パスワードは appsettings.json / ログへの保存を行いません

---

## 現在地（v0.7.0）

- AD ユーザー・グループ・コンピュータの参照
- ユーザー属性限定編集（mail / displayName / sn / givenName）
- コンピュータ description 限定編集
- グループメンバー追加・削除（ユーザーのみ）
- 書き込み監査ログ（write-audit.jsonl）・認証ログ（auth.jsonl）
- **オペレーション支援**（v0.7.0〜）: ユーザー所属変更支援・変更予定サマリー生成・チェックリスト

v0.7.0 は AD 更新対象の拡張ではなく、既存の参照・編集機能を束ねる「定型オペレーション支援」フェーズです。  
実AD環境での動作検証が必要です（`docs/planning/backlog.md` 参照）。

---

## v0.8.0 候補（優先度順・未確定）

1. **GPOシミュレーション**（参照・シミュレーションのみ）
   - 対象種別をユーザー / コンピュータ / ユーザー + コンピュータ から選択
   - 選択種別に応じて sAMAccountName・コンピュータ名を入力
   - シミュレーション実行後、対象に適用予定の GPO 一覧を表示
   - 将来候補: 結果の CSV 出力・クリップボードコピー・参照ログ記録

2. **参照強化**
   - 検索条件の拡張（OU指定・LastLogon日付範囲など）

3. **オペレーション支援の拡張**
   - チェックリスト項目の設定化（OperationChecklistItems の UI 反映）

4. **キャッシュ**（DirectoryReadOnly 実運用後に評価）
   - ユーザー選択切替ごとの `memberOf` 再取得削減
   - セッション内メモリキャッシュ（TTL: 30〜120秒）

5. **ログ検索・ログビューア**
   - `write-audit.jsonl` / `auth.jsonl` の簡易ビューア

6. **UI整理**
   - ボタン配置・ラベルの見直し

7. **ユーザー有効期限更新**
   - `accountExpires` の更新設計継続（`docs/design/design-account-expiration.md` 参照）

8. **編集機能の拡張**（実運用評価後に検討）
   - ユーザー更新対象属性の追加（要件定義次第）

---

## 対象外（今後も実装しない）

- グループ作成・削除・リネーム
- グループをグループに追加
- コンピュータをグループに追加
- GPO 編集・GPO リンク変更・セキュリティフィルタ変更・WMI フィルタ変更
- OU 移動
- ユーザー無効化・退職処理
- パスワードリセット
- 新規ユーザー作成
- サービスアカウント方式（パスワードを設定ファイルに保存する方式）
- 一括更新
