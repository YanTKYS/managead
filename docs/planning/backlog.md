...
## v0.8.x 完了条件（v0.9.0 着手前に確認）

- GPOシミュレーションタブの実AD環境での動作検証（`docs/operation/validation-gpo-simulation.md` 参照）
- ユーザー / コンピュータ / 両方の各種別でシミュレーション実行と結果確認
- 参照ログ（audit.jsonl）に `GpoSimulation` が正しく記録されることを確認
- write-audit.jsonl に更新ログが記録されていないことを確認（AD書き込みなし）

---

## v0.9.x 完了条件（v0.10.0 着手前に確認）

- ログ確認タブの実AD環境での動作検証（`docs/operation/validation-log-viewer.md` 参照）
- 参照ログ / 認証ログ / 書き込みログの読み込み・表示・フィルタリングを確認
- パスワードフィールドのマスクが正常に機能することを確認
- AD書き込みが一切発生しないことを確認（write-audit.jsonl に記録なし）
