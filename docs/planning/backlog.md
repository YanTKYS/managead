...
v0.9.4 で操作説明書・管理者向け設定手順・トラブルシューティングは整備済みです。以降は配布物の最終整理と実 AD 検証結果の反映を優先します。

### 配布・リリース準備

- [ ] 配布 ZIP 構成確認
  - [ ] self-contained 実行ファイルが含まれる
  - [ ] 必要な docs / config-samples が含まれる
  - [ ] 不要な開発用ファイルや一時ファイルが含まれない
- [ ] `release-build.yml` 確認
  - [ ] workflow_dispatch で想定バージョンを指定できる
  - [ ] release-note からタイトル・本文を抽出できる
  - [ ] ZIP artifact 名が想定どおりになる
