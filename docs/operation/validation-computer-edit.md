...
`config-samples/appsettings.ComputerDescriptionEdit.sample.json` を参考に設定する。
...
| 禁止操作 | 確認内容 |
|---------|---------|
| 無効化 | 「コンピュータ参照」タブに Enabled を変更するコントロールがないこと |
| OU移動 | OU 移動ボタン・フィールドがないこと |
| 削除 | 削除ボタンがないこと |
| グループ変更 | グループ追加・削除のコントロールがないこと |
| GPO編集 | GPO関連のコントロールが一切ないこと |
| 一括更新 | 複数コンピュータを一括で更新するUIがないこと |
| パスワードリセット | パスワードリセットのコントロールがないこと |
...
| セッションユーザーと再認証ユーザーの不一致 | セッション開始者と異なるアカウントで再認証 | 「再認証ユーザーと編集セッションユーザーが一致しません」メッセージ |
...
- 検証結果を `docs/operation/test-record-v0.5.0.md` に記録する（別途作成）
