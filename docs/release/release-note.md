...
- **ヘッダーバンドを追加してモードを常時表示**: 画面上部にネイビーのヘッダーバンドを追加し、ツール名「ManageAdTool」・サブタイトル「AD 参照・限定更新支援ツール」・現在のモード（DirectoryReadOnly: 「実AD参照・認証後に限定更新可能」／InMemory: 「テスト用・実AD未接続」）を常時表示する。ヘルプボタンをヘッダー右上へ移動した。
...
- **ウィンドウの最小サイズを設定・レイアウト改善**: MinWidth=900 / MinHeight=720 を設定した。ユーザー詳細タブの差分確認・限定更新実行ボタンを ScrollViewer の外側（常時表示領域）へ移動し、ウィンドウを縮小しても操作ボタンに常にアクセスできるようにした。
- **機能追加は行わず、v1.0.0 からの機能範囲を維持**: 大規模なAD変更（ユーザー無効化・退職処理・OU移動・グループ作成削除・GPO編集）は引き続き行わない。既存の限定更新機能（mail / displayName / sn / givenName / description の属性更新、グループメンバー追加・削除）は変更なし。
