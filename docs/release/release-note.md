...
## v0.9.6
### Title
ManageAdTool v0.9.6（打鍵テスト反映・参照/ログ表示再調整）

### Note
- **打鍵テスト結果を記録**: 初回〜4回目報告の成功・指摘事項を `docs/operation/test-record-v0.9.6.md` に記録した。4回目報告ではグループ検索結果表示とユーザー検索 UI の NG 解消が成功した。
...
- **グループユーザーメンバー表示を修正**: グループ詳細は `memberOf` backlink 検索だけに依存せず、グループの `member` 属性から直接メンバー DN を読み取り、ユーザー/コンピュータ/グループを分類して表示するようにした。
